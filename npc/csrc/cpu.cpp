...
static const char *get_reg_name(uint32_t reg_num) {
...
static void print_execution_info(uint32_t pc, uint32_t inst, bool force_display = false) {
    ...
    // 使用Capstone反汇编引擎获取汇编指令
#ifdef CONFIG_ITRACE
    char *asm_str = disasm_instr(pc, inst);
#else
    // 备用简单反汇编（当CONFIG_ITRACE未定义时）
    char asm_str[128] = "unknown";
    switch (opcode) {
        case 0x03: strcpy(asm_str, "Load指令"); break;
        case 0x23: strcpy(asm_str, "Store指令"); break;
        case 0x63: strcpy(asm_str, "Branch指令"); break;
        case 0x13: strcpy(asm_str, "I-type指令"); break;
        case 0x33: strcpy(asm_str, "R-type指令"); break;
        case 0x37: strcpy(asm_str, "lui指令"); break;
        case 0x17: strcpy(asm_str, "auipc指令"); break;
        case 0x6f: strcpy(asm_str, "jal指令"); break;
        case 0x67: strcpy(asm_str, "jalr指令"); break;
        case 0x73: strcpy(asm_str, "ebreak指令"); break;
    }
#endif
...
static void execute_once(bool force_display = false) 
{
    ...
    // 使用Capstone进行指令反汇编
    char* disasm = disasm_instr(PCSet.pc, PCSet.inst);
    
    // 格式化输出：地址、指令编码、反汇编结果
    p += snprintf(p, sizeof(logbuf) - (p - logbuf), "0x%08x: 0x%08x %s", 
                 PCSet.pc, PCSet.inst, disasm);
    *p = '\0';
#endif
...
static void trace_and_difftest() 
{
    ...
    if(g_print_step) {
        ...
        // 分离地址、指令编码和反汇编
        char* first_space = strchr(logbuf, ' ');
        if (first_space) {
            *first_space = '\0';
            strcpy(addr, logbuf);
            *first_space = ' ';
            ...
            if (second_space) {
                *second_space = '\0';
                strcpy(inst, first_space + 1);
                *second_space = ' ';
                strcpy(disasm, second_space + 1);
                printf("\033[1;90m历史记录: \033[0m%s: \033[1;36m%s\033[0m \033[1;32m%s\033[0m\n", addr, inst, disasm);
            } else {
                strcpy(inst, first_space + 1);
                printf("\033[1;90m历史记录: \033[0m%s: \033[1;36m%s\033[0m\n", addr, inst);
            }
        } else {
